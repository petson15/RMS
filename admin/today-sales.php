...
<nav class="navbar navbar navbar-expand-lg bg-dark navbar-dark push-nav">
  ...
  <div style=" width: 160px; height: 120vh; float: left; position: fixed; display: block;" class="text-white bg-dark nav-side">
    ...
    <h4 class="me-1 my-3 container text-muted ">Today sales</h4>
    <div class="container d-flex justify-content-center sales">
        <table class="table table-bordered text-center fw-lighter" id="order-table">
            <thead class="table-dark text-center fw-lighter">
                <tr>
                    <th scope="col" class="fw-lighter">Employee</th>
                    <th scope="col" class="fw-lighter">Completed orders</th>
                    <th scope="col" class="fw-lighter">Pending prders</th>
                    <th scope="col" class="fw-lighter">Total sales</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr>
                    ...
                    <?php  
                        ...
                        //$query = 'SELECT employee,dates, SUM(price) AS employee_total FROM oders GROUP BY employee';
                    $currentDate = date('Y-m-d');
                    $pending_total = 0;
                    $completed_total = 0;
            //$query = "SELECT employee,completed, SUM(price) AS employee_total  FROM oders WHERE DATE(dates) = '$currentDate'GROUP BY employee";
          ...
          GROUP BY employee";
                    ...
                    while ($rows = mysqli_fetch_assoc($result)) {
                         //code...
                             echo "<td scope='row' class='fw-lighter'>".$rows['employee']."</td>";
                            // code...
                        
                            echo "<td>".$rows['completed_sum']."</td>";
                            echo "<td>".$rows['pending_sum']."</td>";
          ...
          while ($rows = mysqli_fetch_assoc($result)) {
              // code...
             $_SESSION['employeeID'] = $rows['id'];
            echo "

                    <tr scope='row' class='fw-lighter'>
                    <td>TOTAL</td>
                    <td>".$rows['completed_sum']."</td>
                    <td>".$rows['pending_sum']."</td>
                    <td>".$rows['total_sum']."</td>
                </tr>
